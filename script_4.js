...
function getWeatherDetails(name, lat, lon, country, state) {
    let forecast_API_URL = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${api_key}`,
        ...
        .then(data => {
            console.log(data);
            if (data && data.list && data.list.length > 0 && data.list[0].components && data.list[0].main) {
...
fetch(`https://api.openweathermap.org/data/2.5/air_pollution?lat=10&lon=10&appid=${api_key}`).then(res=>res.json()).then(data=>{
    console.log(data)
})
